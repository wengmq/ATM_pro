...
while(1)
{
	...
	case 1:
		{//增加用户
			...
			rt = CommonAdmin_adduser(staff_fp,staff_head);
			if(rt==0)
			{
				system("cls");
				continue;
	...
	case 2:
		{//删除用户
			...
			rt = CommonAdmin_deleteuser(staff_fp,staff_head);
			if(rt==0)
			{
				system("cls");
				continue;
	...
	case 3:
		{//修改用户
			...
			rt = CommonAdmin_modifyuser(staff_fp,staff_head,staff_node);
			if(rt==0)
			{
				system("cls");
				continue;
	...
	case 4:
		{ //查询用户 
			...
			rt = CommonAdmin_queryuser(staff_fp,staff_head);
			if(rt==0)
			{
				system("cls");
				continue;
...
int CommonAdmin_adduser(FILE *staff_fp,LIST *staff_head)
{
	...
	while(1)
	{
		...
		if(rt==2)
			continue;
		if(atoi(iRole)!=3 && atoi(iRole)!=2)
		{
			Gotoxy(28,28);
		    printf("用户身份必须为2或3，请重新输入！");//输入用户身份位
			Sleep(1500); //停滞1.5秒清屏
	        system("cls");
		   continue;
		}
...
int CommonAdmin_deleteuser(FILE *staff_fp,LIST *staff_head)
{
	...
	while(1)
	{
		...
		/*把void类型指针赋值给其他类型的指针，需要强制转化*/
				...
				if(strcmp(sName,data->sName)==0 && (data->iRole == 1 || data->iRole == 3))
				{  //匹配到用户，但对方也是管理员，无法删除
				   flag = -4;
					break;
				}
...
int CommonAdmin_modifyuser(FILE *staff_fp,LIST *staff_head,STAFF *staff_node)
{
	...
	while(1)
	{
		...
		/*把void类型指针赋值给其他类型的指针，需要强制转化*/
				...
				if(strcmp(sName,data->sName)==0 && strcmp(data->sName,staff_node->sName) == 0 )
				{ //账号匹配时，但是对方身份自己时，可以修改
					flag = 1;
					break;
			...
			if(flag == 2)  //查询到用户时,但是对方身份是管理员时，无法修改
			{
			   Gotoxy(30,20);
			   printf("您的权限不足，请重新输入！");
			   Sleep(2000); //停滞1.5秒清屏
